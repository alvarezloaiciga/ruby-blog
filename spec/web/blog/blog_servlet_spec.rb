...
describe "Blog Servlet" do
  describe "do_GET" do
    it "loads all posts titles for the blog" do
      ...
      response.strip.should_equal "<a href=\"posts?blog_id=#{blog.id}&id=#{blog.posts[0].id}\">#{POST_TITLE}</a>\n\n<a href=\"posts?blog_id=#{blog.id}&id=#{blog.posts[1].id}\">#{POST_TITLE_2}</a>"
      stop_test_server
